...
- **Decoration level:** intencional. El rayo (`assets/rayo-*.png`) como forma recurrente (marca de agua, viñetas, chips), garabato ovalado azul en frases de impacto, banda cinética lima. Nada de blobs, glows ni gradientes decorativos ajenos a la marca.
...
- **Accents:** Lima `#CCF020` (`--xp-lime`) y Verde `#82F35C` (`--xp-green`) solo en el bloque de experiencias y en la banda cinética; Cyan `#33CBFF` para marcas y etiqueta "by Freeland"; Lila `#CBCFF0` para texto secundario sobre navy; Naranja `#FFB548` solo en SparkCamp.
- **Neutrals:** fondo `#FFFFFF`, fondo suave `#F2F3FB`, línea `#DCDEF3`, chip `#E8EAFA`, texto `#151519` (nunca negro puro), texto secundario `#55566A`.
...
## Spacing
- **Base unit:** 4px. Escala `--sp-1..8`: 4 · 8 · 12 · 16 · 24 · 32 · 48 · 72.
...
- **Orden de la página:** header → banner (video) → hero → banda cinética → experiencias (programas estrella) → eventos → comunidad → footer al estilo emprendimiento.tec.mx.
...
- **Easing:** `--ease` cubic-bezier(.2,.7,.2,1) para entrar y mover; lineal solo en la banda cinética.
...
- **Perpetuas:** carrusel de video (6s por banner, con botón de pausa), banda cinética (34s por ciclo, se pausa al pasar el cursor), rayo flotante 5s, punto que respira 2.4s, shimmer del esqueleto 1.4s.
